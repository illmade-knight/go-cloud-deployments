...
func main() {
	...
	log.Info().Bool("deployDirector", *runDeployDirector).Msgf("Director URL: %s", *directorURLOverride)
	...
	// --- 3. Hydrate architecture and prepare ServiceDirector source ---

	// This fills in defaults and injects environment variables.
	log.Info().Msg("Hydrating architecture for deployment...")
	err = servicemanager.HydrateArchitecture(&arch, "cloud-deploy", log.Logger.With().Str("step", "hydrate").Logger())
	if err != nil {
	...
	opts := orchestration.ConductorOptions{
		...
		SetupIAM:               *runSetupIAM,
		BuildAndDeployServices: *runDeployServices,
		TriggerRemoteSetup:     *runSetupResources,
		VerifyDataflowIAM:      *runApplyIAM,
